...
export default function OwnerDashboard() {
  ...
  // Maximized garland item & individual order detail state
          ...
          >
            ...
            >
              ...
              <div className="flex-1 overflow-y-auto grid grid-cols-1 md:grid-cols-12 gap-0 divide-y md:divide-y-0 md:divide-x divide-gray-100">
                ...
                {/* Right side: Single Order Details Alone (5 cols) */}
                <div className="md:col-span-5 p-5 sm:p-6 flex flex-col gap-4 bg-white overflow-y-auto">
                  {/* Customer Information (For this order alone) */}
                  <div className="p-4 rounded-2xl bg-gray-50/80 border border-gray-100 shadow-xs">
                    <div className="flex items-center gap-2 text-gray-700 font-bold text-xs uppercase tracking-wider mb-2.5">
                      <User className="w-3.5 h-3.5 text-rose-500" /> Customer Details
                    </div>

                    <div className="flex items-center gap-3">
                      <div className="w-11 h-11 rounded-2xl bg-rose-100 text-rose-700 font-bold text-base flex items-center justify-center shrink-0 shadow-2xs">
                        {maximizedItem.order.customer.full_name.charAt(0).toUpperCase()}
                      </div>
                      <div className="flex-1 min-w-0">
                        <p className="font-bold text-gray-900 text-sm leading-tight">{maximizedItem.order.customer.full_name}</p>
                        <p className="text-xs text-gray-600 font-mono mt-0.5">+91 {maximizedItem.order.customer.phone}</p>
                        {maximizedItem.order.customer.email && (
                          <p className="text-[11px] text-gray-400 truncate mt-0.5">{maximizedItem.order.customer.email}</p>
                        )}
                      </div>
                    </div>
                  </div>

                  {/* Pickup & Payment Details for this order */}
                  <div className="p-4 rounded-2xl bg-jade-50/60 border border-jade-100/80 shadow-xs">
                    <div className="flex items-center justify-between mb-2.5">
                      <span className="text-xs font-bold text-jade-900 uppercase tracking-wider flex items-center gap-1.5">
                        <Calendar className="w-3.5 h-3.5 text-jade-600" /> Pickup & Payment
                      </span>
                      <span className="text-xs font-mono font-bold px-2 py-0.5 rounded-md bg-white text-gray-800 border border-jade-200">
                        #{maximizedItem.order.order_number}
                      </span>
                    </div>
                    ...
                    <div className="space-y-1.5 text-xs">
                      <div className="flex justify-between items-center">
                        ...
                        <strong className="text-gray-900 font-bold">{formatDate(maximizedItem.order.pickup_date)}</strong>
                      </div>
                      <div className="flex justify-between items-center">
                        <span className="text-gray-600">Pickup Time:</span>
                        <strong className="text-jade-800 font-bold">
                          {formatTimeSlot(maximizedItem.order.pickup_start_time, maximizedItem.order.pickup_end_time)}
                        </strong>
                      </div>
                      <div className="flex justify-between items-center pt-1.5 border-t border-jade-100">
                        <span className="text-gray-600">Payment Status:</span>
                        <span className="inline-flex items-center gap-1 font-bold text-emerald-700 font-mono text-xs">
                          <CheckCircle2 className="w-3.5 h-3.5 text-emerald-600" /> PAID ({formatPrice(maximizedItem.order.total)})
                        </span>
                      </div>
                      {maximizedItem.order.payment_method && (
                        <div className="flex justify-between items-center text-[11px] text-gray-400 pt-0.5">
                          <span>Payment Method:</span>
                          <span>{maximizedItem.order.payment_method}</span>
                        </div>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="p-4 rounded-2xl bg-rose-50/50 border border-rose-100 shadow-xs">
                    <p className="text-xs font-bold text-rose-900 uppercase tracking-wider mb-2.5 flex items-center gap-1.5">
                      <ShoppingBag className="w-3.5 h-3.5 text-rose-600" /> Item Ordered
                    </p>

                    <div className="flex items-start justify-between gap-2 mb-1.5">
                      <div>
                        <p className="font-bold text-gray-900 text-sm">{maximizedItem.item.garland_name}</p>
                        <p className="text-xs text-gray-500 mt-0.5">
                          Unit Rate: {formatPrice(maximizedItem.item.unit_price)} each
                        </p>
                      </div>
                      <span className="text-lg font-bold font-mono text-rose-600 shrink-0">
                        × {maximizedItem.item.quantity}
                      </span>
                    </div>

                    <div className="flex items-center justify-between text-xs pt-2 border-t border-rose-100/80">
                      <span className="text-gray-600 font-medium">Item Total:</span>
                      <span className="font-bold text-gray-900 font-mono text-base font-display">
                        {formatPrice(maximizedItem.item.subtotal)}
                      </span>
                    </div>
                  ...
                  <div className="p-3.5 rounded-2xl bg-gray-50 border border-gray-100 flex flex-col gap-2">
                    <p className="text-xs font-semibold text-gray-600">Contact Customer Directly:</p>
                    <div className="flex gap-2">
                      ...
                      <a
                        href={`https://wa.me/91${maximizedItem.order.customer.phone.replace(/\D/g, '')}?text=${encodeURIComponent(
                          `Vanakkam ${maximizedItem.order.customer.full_name}, this is Malligai Garlands regarding your Order #${maximizedItem.order.order_number} for ${maximizedItem.item.garland_name}!`
                        )}`}
                        target="_blank"
                        rel="noreferrer"
                        className="flex-1 py-2.5 px-3 rounded-xl bg-emerald-50 hover:bg-emerald-100 text-emerald-800 border border-emerald-200 font-bold text-xs flex items-center justify-center gap-1.5 transition-colors"
                      >
